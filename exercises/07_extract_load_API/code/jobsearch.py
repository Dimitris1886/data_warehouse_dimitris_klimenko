...
def _get_ads(url_for_search, params): # _get_ads private function
...
def jobsearch_resource(
    ...
    for ad in _get_ads(url_for_search, params)["hits"]:
        yield ad # yield the ads one by one
...
def run_pipeline(query, table_name):
    pipeline = dlt.pipeline(
        ...
        dataset_name='staging', # landing zone for our data
    )
